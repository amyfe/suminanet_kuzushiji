...
def validate_stage2(
    ...
    for batch_idx, batch in enumerate(tqdm(val_loader, desc="Stage2 Validation", leave=False)):
        ...
        losses = compute_stage2_total_loss(
            ...
            lambda_box=STAGE2_LAMBDA_BOX,
            lambda_delta=STAGE2_LAMBDA_DELTA,
            lambda_score=STAGE2_LAMBDA_SCORE,
            lambda_aux=STAGE2_LAMBDA_AUX,
            lambda_decoder=STAGE2_LAMBDA_DECODER,
...
def train_stage2_hybrid(
    ...
    model = build_stage2_model(detector_ckpt_path=detector_ckpt_path, vocab=vocab)
...
def main():
    print(f"===Test: TF hochstellen, AUX hochstellen===")
    ...
    train_stage2_hybrid(
        ...
        lr=LR,
        checkpoint_dir=CHECKPOINT_DIR / "stage2_hybrid_aux",
    )
