...
def print_stats() -> None:
    ...
    suspicious = [(f.stem, frac) for f, frac in zip(npy_files, fracs) if frac > 0.35]
    if suspicious:
        print(f"\nWARNING: {len(suspicious)} image(s) have >35% masked — check these:")
        for stem, frac in sorted(suspicious, key=lambda x: -x[1])[:50]:
...
def main() -> None:
    ...
    parser.add_argument("image", nargs="*", help="Path(s) to input image(s) (omit when using --stats)")
    ...
    if not args.image:
        parser.error("Provide image path(s), or use --stats for dataset-wide summary.")
