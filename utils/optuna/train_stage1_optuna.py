...
from utils.validation.validate_stage1 import compute_detection_metrics, extract_boxes_from_heatmap
...
def objective(trial: optuna.Trial, args: argparse.Namespace) -> float:
    ...
    # Predeclared so the `finally` block below can always `del` them, even
    # if an exception fires partway through construction.
    train_loader = val_loader = None
    backbone = detector = optimizer = scheduler = scaler = None
    try:
        ...
        for epoch in range(args.epochs):
            ...
            # Study direction is "maximize" under --optimize-f1 (final return value is F1),
            # but val_loss is lower-is-better — negate it so the pruner's median comparison
            # stays consistent with the study direction instead of pruning the best trials.
            trial.report(-val_loss if getattr(args, "optimize_f1", False) else val_loss, step=epoch)
        ...
        del backbone, detector, optimizer, scheduler, scaler
        release_cuda_memory()
