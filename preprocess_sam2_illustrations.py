...
def _collect_entries(data_dir: Path, splits: List[str]) -> List[dict]:
    ...
    Return one dict per image:
      {
        "img_path":  Path,
        "stem":      str,          # image filename stem (used as mask file key)
...
def run_preprocessing(
    ...
    *,
    ...
    """
    Build (or extend) the SAM2 illustration mask cache for the given splits.

    Saves one bool .npy file per image at IMAGE_SIZE resolution.
    Already-cached images are skipped unless overwrite=True.
    """
    import torch
    ...
    for entry in tqdm(pending, desc="[SAM2]"):
        ...
        illus_mask = build_illustration_mask(
            img_rgb, masks_data, entry["gt_boxes"], entry["orig_size"],
            area_thresh, area_max_thresh, min_area, min_overlap_frac,
        )

        # Resize to training resolution using nearest-neighbour.
        # illus_mask is float32 [0,1]; PIL mode 'F' handles float32 images.
        if illus_mask.max() > 0.01:
            pil_mask = Image.fromarray(illus_mask, mode="F").resize(
